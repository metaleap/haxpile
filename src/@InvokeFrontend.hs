...
invokeGhc   :: Invoke  ->  String  ->  [String]
            ->  IO ()
...
invokeGhc PrepGhc stackghciscriptfilepath args
    = moduleNames stackghciscriptfilepath >>= \modnames
    -> invokeGhc RunGhc "" (modnames ++ nuargs)
    where
    nuargs = (args ~|not.ditch) ++
...
moduleNames ghciscriptfilepath
    ...
    -> if not isfile then pure [] else
        ...
        --------often just:
        -- :module +
        in pure$ (lines content) ~| (Lst.isPrefixOf ":module ")
        >>= ((keep|~) . Lst.splitOn ' ' . drop 8)
